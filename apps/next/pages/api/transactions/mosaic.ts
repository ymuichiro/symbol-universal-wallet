...
async function getHandle(req: NextApiRequest, res: NextApiResponse) {
    let { signerPublicKey, duration, divisibility, flags, action, delta, feeMultiplier, networkType, deadline } = req.query;
    ...
    const address = facade.network.publicKeyToAddress(new symbolSdk.PublicKey(symbolSdk.utils.hexToUint8(signerPublicKey)));
